...
3. Safety assistant over OSHA Severe Injury Reports. Done: provenance, a
   ...
   The assistant was also deployed as an agent (a bounded demo, deleted
   afterwards): an MLflow `ResponsesAgent` on a scale-to-zero Model Serving
   endpoint with the Review App. Rerunning the identity and v2 question sets
   through the endpoint gave the same decisions and citations as in-process.
